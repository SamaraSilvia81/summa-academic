...
export function ReferenceGraph({ references, profileId, selectedNode, onSelectNode }) {
  ...
  const { nodes, edges } = useMemo(() => buildGraph(references, { maxNodes: 30 }), [references]);
  ...
  const labeledIds = useMemo(() => {
    return new Set([...nodes].sort((a, b) => b.degree - a.degree).slice(0, 8).map(n => n.id));
  }, [nodes]);
  ...
  if (nodes.length === 0) {
    return (
      <div style={{
        ...
        gap: 10, padding: '100px 20px', color: 'var(--tx3)', textAlign: 'center',
      }}>
        <div style={{
          ...
          <GraphIcon size={22} weight="duotone" color="var(--acc)" />
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, maxWidth: 300, lineHeight: 1.6 }}>
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{
        ...
        flexWrap: 'wrap', gap: 8, padding: '0 8px 6px',
      }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {legendTypes.map(([type, color]) => (
            <span key={type} style={{
              ...
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, boxShadow: `0 0 6px ${color}66` }} />
              {TYPE_LABELS[type] || type}
            </span>
          ))}
        </div>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--tx3)' }}>
          {nodes.length} nós · {edges.length} conexões
        </span>
      </div>

      {/* ── SVG graph area (no border, no card, full bleed) ── */}
      <div
        ref={wrapRef}
        ...
        style={{
          ...
          userSelect: 'none',
          background: 'radial-gradient(ellipse 65% 55% at 50% 45%, rgba(212,160,48,0.04), transparent 70%)',
        }}
      >
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
          <defs>
            ...
            <pattern id="dotGrid" width="28" height="28" patternUnits="userSpaceOnUse">
              <circle cx="1" cy="1" r="0.8" fill="var(--tx3)" opacity="0.12" />
            </pattern>
          </defs>
            ...
            {edges.map((e, i) => {
              ...
              return (
                <path key={i}
                  ...
                  strokeOpacity={dim ? 0.25 : 0.35 + Math.min(e.weight, 4) * 0.1}
                  strokeWidth={dim ? 0.8 : 1 + Math.min(e.weight, 4) * 0.4}
                />
              );
            })}
              ...
              return (
                ...
                >
                  ...
                  <circle r={r}
                    ...
                    style={{ filter: dim ? 'none' : `drop-shadow(0 0 6px ${n.color}55)`, transition: 'fill-opacity 0.15s' }}
                  />
                  {n.ref.isFavorite && (
                  ...
                  {showLabel && (
                    <>
                      <rect x={-2} y={r + 4} width={Math.min(n.ref.title.length * 4.5 + 8, 140)} height={14}
                        fill="var(--bg0)" fillOpacity={0.82} rx={3} />
                      <text x={2} y={r + 14} fontSize="9" fontFamily="var(--font-mono)" fill="var(--tx2)"
                        style={{ pointerEvents: 'none' }}>
                        {n.ref.title.length > 28 ? n.ref.title.slice(0, 26) + '…' : n.ref.title}
                      </text>
                    </>
                  )}
                </g>
        ...
        {/* ── Controles de zoom (glass) ── */}
        <div style={{
          position: 'absolute', bottom: 10, right: 10, display: 'flex', flexDirection: 'column',
          gap: 1, background: 'var(--glass-bg-strong)', backdropFilter: 'blur(12px)',
          WebkitBackdropFilter: 'blur(12px)',
        ...
        <div style={{
          ...
          fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--tx3)', opacity: 0.6,
        }}>
          arraste pra mover · scroll pra zoom
        </div>

        {/* ── Detail panel (floating glass, appears on node click) ── */}
        {selected && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>
              <span style={{
                fontFamily: 'var(--font-mono)', fontSize: 8.5, color: selected.color,
                background: selected.color + '15', border: `1px solid ${selected.color}28`,
            ...
            <div style={{
              fontFamily: 'var(--font-display)', fontSize: 13, fontWeight: 700, color: 'var(--tx)',
              marginTop: 8, lineHeight: 1.35,
            }}>{selected.ref.title}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--tx3)', marginTop: 3 }}>
            ...
            {selected.ref.tags?.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 10 }}>
                {selected.ref.tags.slice(0, 5).map(t => (
                  <span key={t} style={{
                    fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'var(--tx2)',
                    background: 'var(--glass-border)', border: '1px solid var(--glass-border)',
                    borderRadius: 3, padding: '2px 5px', display: 'flex', alignItems: 'center', gap: 2,
                  }}><TagSimple size={8} />{t}</span>
                ))}
              </div>
            )}
            ...
            <div style={{
              ...
              <span style={{ color: 'var(--acc)', fontWeight: 700 }}>{selected.degree}</span> conexões
              {selected.ref.isFavorite && <span style={{ color: '#D4A030', marginLeft: 4, display: 'flex', alignItems: 'center', gap: 2 }}><Star size={8} weight="fill" /> fav</span>}
            </div>

            {topConnections.length > 0 && (
              <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid var(--glass-border)' }}>
                <div style={{
                  fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--tx3)',
                  textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6,
                }}>conectadas</div>
                {topConnections.map(({ node, shared }) => (
                  <button key={node.id} onClick={() => handleSelectNode(node)} style={{
                    display: 'block', width: '100%', textAlign: 'left', background: 'none',
                    border: 'none', padding: '4px 0', cursor: 'pointer',
                  }}>
                    <div style={{
                      fontFamily: 'var(--font-body)', fontSize: 10.5, color: 'var(--tx2)',
                      overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                    }}>{node.ref.title}</div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--tx3)', marginTop: 1 }}>
            ...
            <button onClick={() => openInAcervo(selected.ref.id)} style={{
              ...
              fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--bg0)',
              background: 'var(--acc)', border: 'none', borderRadius: 7,
              padding: '7px 10px', cursor: 'pointer', fontWeight: 600,
              boxShadow: '0 0 12px var(--acc-glow)',
            }}>
              abrir no acervo <ArrowSquareOut size={10} />
            </button>
          </div>
        )}
      </div>
